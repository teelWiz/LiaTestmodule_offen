...
icon: cau-norm-de-lilagrey-rgb-0720.png
...
<!-- //TODO vom 18.März: Durchkommentieren und pushen -->
...
Herzlich Willkommen im Lernbaustein "**Digitale Forschungsdaten ordnen und strukturieren**"!
 ...
 ![Bild](FDM_Zyklus_klein.jpg)<!--  width="250px"
  height="250px"
  align="right"-->
...
* Benennungskonventionen zu entwickeln und auf eigene sowie kollaborativ genutzte (Forschungsdaten-)Dateien anzuwenden.
...
2. [Pfade und Pfadnamen](#8)
3. [Ordnerstrukturen: Ordner so anlegen, dass man sie wiederfindet und weiß, was drin ist.](#7)
4. [Dateinamen: Dateien so benennen, dass man versteht was sie enthalten](#12)
5. [Dokumentieren: Dateiablage und Benennungskonventionen so beschreiben, dass man sich (auch in Zukunft noch) zurechtfindet](#16)
...
![Bild](2022-03-08_Datenverlust_Comic-v2_CM.jpg) <!--  width="450px"
 height="450px"
-->
...
Ein strukturiertes Vorgehen beim Anlegen und Verwalten von Ordnerstrukturen sowie bei der Vergabe von Datenamen verfolgt folgende Ziele:
>* Einfacheres und schnelleres Suchen und Finden von Dateien,
>* Vermeidung doppeltete Arbeit,
...
![Bild](2022-03-23_Icon_Puzzel.jpg) <!--  width="80px"
 height="80px"
 align="right"-->
...
>**Je nach genutzem Betriebssystem besteht ein Pfadname aus den Elementen:**
>
...
>
> Als Trennzeichen nutzt Windows den umgekehrter Schrägstrich (\) in Pfaden.
>
...
>
>![](2022_Pfadbeispiel.png "Hier steht **C:\\** für das Laufwerk/den Datenträger, auf dem die Datei abgelegt ist. Auf diesem Laufwerk C liegt ein Ordner **uni**, in welchem sich ein Ordner **physik_I** befindet. Innerhalb des Ordners physik_I befindet sich ein Ordner **protokolle**, in welchem eine Datei mit dem Namen **beispielprotokoll.docx** abgelegt ist. (Abbildung: Britta Petersen)")<!--  width="350px" align="middle"-->

>**Unix** und **Unix-Derivate** (z. B. Linux) arbeiten mit einem sogenannten „Wurzelverzeichnis“ (root directory). Von diesem Wurzelverzeichnis aus wird rein verzeichnisorientiert, ohne Angabe eines Laufwerkes/Datenträgers adressiert. Aus einer Unix-Pfadangabe alleine ist daher nicht ersichtlich, auf welchem physikalischen Datenträger eine bestimmte Datei liegt. In der Regel liegt das Wurzelverzeichnis auf dem Datenträger, von welchem der Systemstart erfolgt. Weitere Datenträger werden durch die Verwaltung von sogennanten Einhhängepunkten (mount points) eingebunden.
>
...
>
>![](2022_Pfadbeispiel_Unix.png "Hier liegt ein Ordner **uni** im Wurzelverzeichnis, in welchem sich ein Ordner **physik_I** befindet. Innerhalb des Ordners physik_I befindet sich ein Ordner **protokolle**, in welchem eine Datei mit dem Namen **beispielprotokoll.docx** abgelegt ist. Es ist nicht ersichtlich, auf welchem physikalischen Datenträger die Datei **beispielprotokoll.docx** gespeichert ist. (Abbildung: Britta Petersen)")<!--  width="350px" align="middle"-->
...
[[X]] Ein Pfad besteht aus den Elementen "Laufwerks- oder Datenträgerbezeichnung", "Liste von Ordnern bzw. (Unter-)Ordnern" und "Dateinamen".
...
>![](2022-02-23_Icon_Glühbirne.jpg)<!--  width="80px" align="right"-->
>
...
Entwerfen Sie eine hirarchisch organisierte Ordnerstruktur. Beginnen Sie mit einer begrenzten Anzahl von Ordnern für die übergeordneten Themen und legen Sie dann innerhalb dieser Ordner weitere spezifischere Ordner an.

Folgendes Beispiel stellt eine hirachische Strukturierung dar:
...
Ihr Ablagekonzept sollte besser einfach als komplex sein.
...
Um ein versehendliches Bearbeiten zu verhindern, können Sie sich selbst die die Schreibrechte für die Rohdaten-Dateien entziehen.
...
Überprüfen Sie auch im weiteren Verlauf, ob Ihre Ablagestruktur noch passt. Ein Hinweis auf Überarbeitungsbedarf könnte die Zahl der Dateien sein, die nicht eindeutig zugeordnet werden können (und daher ggf. in der Zwischenablage landen).
...
Versuchen Sie nach Möglichkeit, sich möglichst früh ein Ordnungsschema zu überlegen. Dokumentieren Sie Ihr Ordnungs- und Bennungsschema!
...
Entwickeln Sie eine gemeinsame Ordnerstruktur, eine gemeinsame Dateinbenennungskonvention und legen Sie Zugriffsrrechte fest.
...
![](2022_OrdnerstrukturUebungXY.png "Beispielhafte Orderstruktur für eine Übung, in welcher Daten erhoben, ausgewertet und in einer Hausarbeit und/oder einem Poster zusammengefasst und beschrieben werden.  (Abbildung: Britta Petersen)")<!--  width="250px" align="center"-->

---

![](2022_OrdnerstrukturArchproject.png "Beispielhafte Orderstruktur für ein wissenschaftliches Projekt aus dem Fachbereich Ur- und Frühgeschichte. (Abbildung: Britta Petersen nach Angaben von Oliver Nakoinz)")<!--  width="250px" align="center"-->
...
**Ordner sollten hierarchisch strukuriert werden. Warum sollten jedoch nicht übermäßig viele Unterordner angelegt werden?**
...
![Bild](phd_final.gif)<!--  width="250px"
 height="200px"
 align="right"-->
Wild vergebene Dateinamen können uns um den Verstand bringen! Beispielsweise, wenn wir um 3:00 Uhr nachts verzweifelt nach der wirklich aktuellen Version der Hausarbeit suchen, die wir spätestens morgen früh um 9:00 Uhr abgeben müssen! Schlimmer noch, wenn  uns auffällt, dass wir leider die Version abgegeben haben, in der diese eine wichtige Änderung noch nicht eingearbeitet war. Oder wenn wir versehendlich eine aktuelle Version, die umfangreiche Änderungen enthielt, mit einer älteren Variante überschreiben.  

Es empfiehlt sich daher, klare Regeln aufzustellen, nach denen Dateien benannt werden. Dies gilt umso mehr, wenn Dateien von mehreren Personen gemeinsam angelegt und bearbeitet werden.

Es ist nicht möglich, zwei Dateien mit exakt dem selben Namen in ein und demselben Ordner zu speichern, da die Pfadnamen beider Dateien in einem solchen Fall identisch wären. Zwei Dateien mit einer exakt identischen Benennung in zwei unterschiedlichen Ordnern zu speichern, ist dagegen theoretisch möglich, da in dem Fall die Pfadnamen beider Dateien weiterhin individuell sind. Empfehenswert ist eine identische Dateibenennung in unterschiedlichen Ordnern jedoch nicht, da dann viel zu sehr darauf geachtet werden muss, an welcher Datei genau gerade gearbeitet wird.
...
| [Institutionenkennung]\_[Sammlungskennung]\_[Merkmal]\_[Inventarnummer]\_[Aufnahmekennung]\_[Typenkennung].Dateiendung | sgs\_mup\_creed\_a-123-dmg-12-54\_uv1\_m.tif |
...
**Wie dürfen Elemente eines Dateinamens im Sinne einer besseren Lesbarkeit getrennt werden? (Mehrfachauswahl möglich)** (Mehrfachauswahl möglich)
...
**README-Dateien** sollten stets zusammen mit den Dateien abgelegt werden, für die die jeweilige Dokumentation gelten soll. Ein guter Speicherort ist die jeweils oberste Ordnerebene (vgl. Beispiele für mögliche Ordnerstrukturen).
...
>Arbeitsblatt "Dokumentation von Dateibenennungskonventionen" im txt-Format: <A HREF="2022_AB_DokuDateibenennung.txt">2022\_AB\_DokuDateibenennung.txt</A>
